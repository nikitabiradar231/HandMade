...
export function StatusBanner({ status }: { status: Status | null }) {
  ...
  const isSpinning =
    ...
    status.kind === 'submitted_waiting_for_index';
      ...
      <div className="min-w-0">
        ...
        {status.kind === 'proving' && (
          <p className="mt-1 text-xs text-amber-600 font-medium">
            Generating zero-knowledge proof… please keep this tab open (30–60s).
          </p>
        )}
        {status.kind === 'submitted_waiting_for_index' && (
          <p className="mt-1 text-xs text-purple-700 font-medium">
            Blockchain transaction broadcast. Waiting for Midnight network block indexing…
          </p>
        )}
      </div>
    </div>
